...
const port = 9334;
const userData = join(tmpdir(), `react-storefront-1b-${Date.now()}`);
const outDir = "C:/Users/andre/Desktop/Proyectos/react-storefront/docs/screenshots/phase-1b";
const productUrl = "http://localhost:5173/item/1z2QAm0L5YXdEHOz20Ga";
const cartUrl = "http://localhost:5173/cart";
const checkoutUrl = "http://localhost:5173/formulario";
...
async function setupProductDetail(page) {
  await page.send("Page.navigate", { url: productUrl });
  await waitForSelector(page, "[class*='imagePanel'] img");
}
...
try {
  await capture("product-detail-desktop.png", 1440, 1000, false, setupProductDetail);
  await capture("product-detail-mobile.png", 390, 1200, true, setupProductDetail);
